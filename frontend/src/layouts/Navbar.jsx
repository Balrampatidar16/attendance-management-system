...
export default function Navbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between h-16 px-4 md:px-6 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur">
      <button
        type="button"
        onClick={onMenuClick}
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-red-600 dark:hover:text-red-400 disabled:opacity-50"
        >
          Logout
        </button>
      </div>
